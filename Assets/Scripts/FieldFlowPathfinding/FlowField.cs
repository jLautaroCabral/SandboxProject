...
public class FlowField
{
    ...
    public void CreateIntegrationField(Cell _destinationCell)
    {
        ...
        Queue<Cell> cellsToChech = new Queue<Cell>();
        cellsToChech.Enqueue(destinationCell);

        while(cellsToChech.Count > 0)
        {
            Cell curCell = cellsToChech.Dequeue();
            List<Cell> curNeighbors = GetNeigborCells(curCell.gridIndex, GridDirection.CardinalDirections);
            ...
            foreach(Cell curNeighbor in curNeighbors)
            {
                ...
                if(curNeighbor.cost + curCell.bestCost < curNeighbor.bestCost)
                {
                    ...
                    cellsToChech.Enqueue(curNeighbor);
                }
            }
    ...
    public void CreateFlowField()
    {
        foreach(Cell curCell in grid)
        {
            List<Cell> curNeighbors = GetNeigborCells(curCell.gridIndex, GridDirection.AllDirections);
    ...
    private List<Cell> GetNeigborCells(Vector2Int nodeIndex, List<GridDirection> directions)
    {
        List<Cell> neighborCells = new List<Cell>();

        foreach(Vector2Int curDirection in directions)
        {
            Cell newNeigbor = GetCellAtRelativePos(nodeIndex, curDirection);
            if (newNeigbor != null)
                neighborCells.Add(newNeigbor);
        }

        return neighborCells;
    }

    private Cell GetCellAtRelativePos(Vector2Int origPos, Vector2Int relativePos)
    {
        Vector2Int finalPos = origPos + relativePos;
