...
//FACTION
//Made spawn location public

//TILE MASTER CLASS
//MADE LOCATION IN WORLD PUBLIC SO WE CAN SEE IT IN INSPECTOR

//POTENTIAL BUILDING LOCATION
//self explanatory

//CONSTRUCTION JOB STORE
//stores workers, building and 

public class AIBuildingStore : MonoBehaviour {
	public List<GameObject> myBuildings;
	Faction myFaction;

	//variables for the threaded building location find
	ThreadedBuildingLocationFind tblf;
	...
	{
		tblf = new ThreadedBuildingLocationFind ();
	...
	void createNextBuilding()
	{
		if (tilesFromThread.Count > 0) { //if we can't find a place for it then we don't create a building
			...
			if (myJobs [0].toBuild.name.Equals ("House")) {
				myFaction.myPopulation.addHouse (g);
			}
			/////
			/////
			/////
			...
			foreach (GameObject workers in myJobs[0].workersToBuild) {
				AI_BuildBuilding build = workers.AddComponent<AI_BuildBuilding> ();
				build.initaliseTarget (g);

				UnitMasterClass umc = workers.GetComponent<UnitMasterClass> ();
				umc.interruptQueue (build);//actionsQueue.Add (build);
			}
			//myBuildings.Add (g);
		}
	...
	GameObject createBuilding()
	{
		...
		Color cl = new Color (1, 1, 1, 0.05f);
		sr.color = cl;
	...
	{
		List<GameObject> workers = myFaction.myUnits.getFreeWorkers ();

		myJobs.Add(new ConstructionJobStore(posInWorld,toBuild,workers));
	}
