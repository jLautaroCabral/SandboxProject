...
namespace LC.Debug
{
    public static class LC_Debug
    {
        ...
        {
            CameraSwitcher.SetupCurrentCamera(LC_FreeLookCamera.CreateFreeLookCamera());
        }
        public static void UseRotateAroundTargetCamera(Transform target)
        {
            CameraSwitcher.SetupCurrentCamera(LC_RotateAroundCamera.CreateRotateAroundToTargetCamera(target));
        }
        ...
        private static class LC_GraphicsDebugUtils
        {
            ...
            public static GameObject CreateButtonPanel(Transform parent, Vector3 position)
            {
                ...
                LC_Debug.RuntimeDebugObjectsManager.debugObjects.Add(btnPanelObj);
            ...
            {
                return DrawTextUI(textString, LC_Utils.GetCanvasTransform(), anchoredPosition, fontSize, font);
            }
            ...
            public static Text DrawTextUI(string textString, Transform parent, Vector2 anchoredPosition, int fontSize, Font font)
            {
                ...
                if (font == null) font = LC_Utils.GetDefaultFont();
                text.font = font;
                text.fontSize = fontSize;

                LC_Debug.RuntimeDebugObjectsManager.debugObjects.Add(textGo);
        ...
        public static class RuntimeDebugObjectsManager
        {
            ...
            public static void HandleDebugObjectsFarFromCamera()
            {
                foreach (GameObject debugObj in debugObjects)
                {
                    if(Vector3.Distance(debugObj.transform.position, CameraSwitcher.CurrentCamera.transform.position) > LC_Utils.GetSettings().DebugObjectsDrawDistance)
                    {
                        debugObj.SetActive(false);
                    } else
                    {
